...
#pharma
  # Fetch stock data for pharma stocks
    pharma_company_names, pharma_stock_data = fetch_stock_data(pharma_stock)
    valid_pharma_stock = [stock for stock in pharma_stock if stock in pharma_stock_data]
    
    # Check if valid_pharma_stocks is empty
    if not valid_pharma_stock:
        pharma_plot_url = None
    else:
        pharma_plot_url = create_plot(pharma_company_names, pharma_stock_data, valid_pharma_stock)

#IT_Sector

    it_company_names, it_stock_data = fetch_stock_data(it_stock)
    valid_it_stock = [stock for stock in it_stock if stock in it_stock_data]
    
    # Check if valid_it_stocks is empty
    if not valid_it_stock:
        it_plot_url = None
    else:
        it_plot_url = create_plot(it_company_names, it_stock_data, valid_it_stock)
...
# Fetch stock data for american stocks
    ...
    return render_template('index.html', 
						   ...
						   fmcg_amc_plot_url=fmcg_amc_plot_url,
                            pharma_plot_url= pharma_plot_url,
                            it_plot_url=it_plot_url)
